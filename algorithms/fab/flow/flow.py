...
class Flow(NamedTuple):
    ...
    sample_and_log_prob_apply: Callable[
        [FlowParams, chex.PRNGKey, chex.Shape], Tuple[Sample, LogProb]
    ]
    ...
    sample_and_log_prob_with_extra_apply: Callable[
        [FlowParams, chex.PRNGKey, chex.Shape], Tuple[Sample, LogProb, Extra]
    ]
    config: Any
    dim: int
...
def create_flow(recipe: FlowRecipe) -> Flow:
    ...
    forward_and_log_det_with_extra_single = FlowForwardAndLogDetWithExtra(
        bijector=bijector_block
    )
    inverse_and_log_det_with_extra_single = FlowInverseAndLogDetWithExtra(
        bijector=bijector_block
    )
    ...
    def log_prob_apply(params: FlowParams, sample: Sample) -> LogProb:
        ...
        (x, log_det), _ = jax.lax.scan(
            scan_fn,
            init=(sample, jnp.zeros(log_prob_shape)),
            xs=params.bijector,
            reverse=True,
    ...
    def log_prob_with_extra_apply(
        params: FlowParams, sample: Sample
    ) -> Tuple[LogProb, Extra]:
        def scan_fn(carry, bijector_params):
            y, log_det_prev = carry
            x, log_det, extra = inverse_and_log_det_with_extra_single.apply(
                bijector_params, y
            )
            chex.assert_equal_shape((log_det_prev, log_det))
        ...
        for i in reversed(range(recipe.n_layers)):
            info.update(
                {f"block{i}_" + key: val[i] for key, val in extra.aux_info.items()}
            )
            aggregators.update(
                {f"block{i}_" + key: val for key, val in extra.info_aggregator.items()}
            )

        info.update(mean_base_log_prob=jnp.mean(base_log_prob))
        aggregators.update(mean_base_log_prob=jnp.mean)
        extra = Extra(
            aux_loss=extra.aux_loss, aux_info=info, info_aggregator=aggregators
        )

        return base_log_prob + log_det, extra

    def sample_and_log_prob_apply(
        params: FlowParams, key: chex.PRNGKey, shape: chex.Shape
    ) -> Tuple[Sample, LogProb]:
        def scan_fn(carry, bijector_params):
    ...
    def sample_and_log_prob_with_extra_apply(
        params: FlowParams, key: chex.PRNGKey, shape: chex.Shape
    ) -> Tuple[Sample, LogProb, Extra]:
        def scan_fn(carry, bijector_params):
            x, log_det_prev = carry
            y, log_det, extra = forward_and_log_det_with_extra_single.apply(
                bijector_params, x
            )
            chex.assert_equal_shape((log_det_prev, log_det))
    ...
    def init(seed: chex.PRNGKey, sample: Sample) -> FlowParams:
        ...
        params_bijectors = jax.tree_util.tree_map(
            lambda x: jnp.repeat(x[None, ...], recipe.n_layers, axis=0),
