...
def build_metropolis(
    ...
    def step(
        ...
        def one_step(point: Point, key: chex.PRNGKey) -> Tuple[Point, float]:
            ...
            new_x = (
                point.x
                + jax.random.normal(key1, shape=point.x.shape)
                * transition_operator_state.step_size
            )
            new_point = create_point(
                new_x, log_q_fn=log_q_fn, log_p_fn=log_p_fn, with_grad=False
            )
            chex.assert_trees_all_equal_shapes(point, new_point)

            log_p_accept = get_intermediate_log_prob(
                log_q=new_point.log_q, log_p=new_point.log_p, beta=beta, alpha=alpha
            ) - get_intermediate_log_prob(
                log_q=point.log_q, log_p=point.log_p, beta=beta, alpha=alpha
            )
            log_threshold = -jax.random.exponential(key2)

            accept = (
                (log_p_accept > log_threshold)
                & jnp.isfinite(new_point.log_q)
                & jnp.isfinite(new_point.log_p)
            )
            point = jax.lax.cond(
                accept, lambda p_new, p: p_new, lambda p_new, p: p, new_point, point
            )
        ...
        point, mean_p_accept = jax.lax.scan(
            scan_fn, point, jax.random.split(subkey, n_steps)
        )
        mean_p_accept = jnp.mean(mean_p_accept)

        if tune_step_size:
            step_size = jax.lax.cond(
                mean_p_accept > target_p_accept,
                lambda step_size: step_size * step_size_multiplier,
                lambda step_size: step_size / step_size_multiplier,
                transition_operator_state.step_size,
            )
            transition_operator_state = transition_operator_state._replace(
                step_size=step_size
            )
        ...
        info = {}
        info.update(
            mean_p_accept=mean_p_accept, step_size=transition_operator_state.step_size
        )
