...
class IgnoreNanOptState(NamedTuple):
    ...
    total_steps: chex.Array = jnp.array(
        0, dtype=int
...
def dynamic_update_ignore_and_grad_norm_clip(
    ...
    def update(
        grad: chex.ArrayTree, opt_state: IgnoreNanOptState, params: chex.ArrayTree
...
def get_optimizer(optimizer_config: OptimizerConfig):
    ...
    main_grad_tranform = getattr(optax, optimizer_config.optimizer_name)(
        lr
    )  # e.g. adam.
    ...
    if optimizer_config.dynamic_grad_ignore_and_clip:
        ...
        if optimizer_config.max_global_norm:
            clipping_fn = optax.clip_by_global_norm(
                float(optimizer_config.max_global_norm)
            )
            grad_transforms.append(clipping_fn)
