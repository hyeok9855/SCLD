...
def sample_without_replacement(
    key: chex.Array, logits: chex.Array, n: int
) -> chex.Array:
...
class InitFn(Protocol):
    def __call__(
        self, x: chex.Array, log_w: chex.Array, log_q_old: chex.Array
...
class SampleFn(Protocol):
    def __call__(
        self, key: chex.PRNGKey, buffer_state: PrioritisedBufferState, batch_size: int
...
def build_prioritised_buffer(
    ...
    def init(
        x: chex.Array, log_w: chex.Array, log_q_old: chex.Array
    ) -> PrioritisedBufferState:
        """
        Initialise the buffer state, by filling it above `min_sample_length`.
        """
        ...
        data = Data(
            x=jnp.zeros((max_length, dim)) * float("nan"),
            log_w=-jnp.ones(
                max_length,
            )
            * float("inf"),
    ...
    def add(
        ...
        x, log_w, log_q = jax.tree_util.tree_map(
            ...
            (x, log_w, log_q),
            (
                buffer_state.data.x[indices],
                buffer_state.data.log_w[indices],
    ...
    def sample(
        key: chex.PRNGKey, buffer_state: PrioritisedBufferState, batch_size: int
    ) -> Tuple[chex.Array, chex.Array, chex.Array]:
        """
        Sample a batch from the buffer in proportion to the log weights.
        Returns:
        ...
        if sample_with_replacement:
            indices = jax.random.categorical(
                key, buffer_state.data.log_w, shape=(batch_size,)
            )
        else:
            key1, key2 = jax.random.split(key)
            indices = sample_without_replacement(
                key1, buffer_state.data.log_w, batch_size
            )
            indices = jax.random.permutation(key2, indices)
