...
def generic_loss(
    ...
    """
    info = {}
    fab_loss, (log_w_adjust, log_q) = fab_loss_buffer_samples_fn(
        params, x, log_q_old, alpha, flow.log_prob_apply, w_adjust_clip
    )
    info.update(fab_loss=fab_loss)
    if use_reverse_kl_loss:
        assert target_log_prob is not None
        batch_size = x.shape[0]
        rkld_loss = reverse_kl_loss_fn(
            params, key, batch_size, flow.sample_and_log_prob_apply, target_log_prob
        )
...
def build_fab_with_buffer_init_step_fns(
    ...
    def init(key: chex.PRNGKey) -> TrainStateWithBuffer:
        ...
        def body_fn(carry, xs):
            ...
            chex.assert_rank(
                x0, 2
            )  # Currently written assuming x only has 1 event dimension.
            point, log_w, smc_state, smc_info = smc.step(
                x0, smc_state, log_q_fn, log_p_fn
            )
            return smc_state, (point.x, log_w, point.log_q)

        n_forward_pass = (buffer.min_lengtht_to_sample // batch_size) + 1
        smc_state, (x, log_w, log_q) = jax.lax.scan(
            body_fn, init=smc_state, xs=jax.random.split(key4, n_forward_pass)
        )

        buffer_state = buffer.init(
            jnp.reshape(x, (n_forward_pass * batch_size, flow.dim)),
            log_w.flatten(),
            log_q.flatten(),
        )

        return TrainStateWithBuffer(
    ...
    def one_gradient_update(
        carry: Tuple[FlowParams, optax.OptState],
        ...
        info.update(
            log10_grad_norm=jnp.log10(grad_norm)
        )  # Makes scale nice for plotting
        info.update(log10_max_param_grad=jnp.log(jnp.max(ravel_pytree(grad)[0])))
        if isinstance(opt_state, IgnoreNanOptState):
            info.update(
                ignored_grad_count=opt_state.ignored_grads_count,
    ...
    def step(state: TrainStateWithBuffer) -> Tuple[TrainStateWithBuffer, Info]:
        ...
        x_buffer, log_q_old_buffer, indices = buffer.sample_n_batches(
            subkey, state.buffer_state, batch_size, n_updates_per_smc_forward_pass
        )
        # Perform sgd steps on flow.
        key, subkey = jax.random.split(key)
        (new_flow_params, new_opt_state), (infos, log_w_adjust, log_q_old) = (
            jax.lax.scan(
                one_gradient_update,
                init=(state.flow_params, state.opt_state),
                xs=(
                    x_buffer,
                    log_q_old_buffer,
                    jax.random.split(subkey, n_updates_per_smc_forward_pass),
                ),
                length=n_updates_per_smc_forward_pass,
            )
        )
        # Adjust samples in the buffer.
        buffer_state = buffer.adjust(
            log_q=log_q_old.flatten(),
            log_w_adjustment=log_w_adjust.flatten(),
            indices=indices.flatten(),
            buffer_state=state.buffer_state,
        )
        # Update info.
        for i in range(n_updates_per_smc_forward_pass):
            info.update(jax.tree_util.tree_map(lambda x: x[i], infos))
        ...
        point, log_w, smc_state, smc_info = smc.step(
            x0, state.smc_state, log_q_fn, log_p_fn
        )
        info.update(smc_info)
        buffer_state = buffer.add(
            x=point.x, log_w=log_w, log_q=point.log_q, buffer_state=buffer_state
        )

        new_state = TrainStateWithBuffer(
