...
from utils.nice_trainer.nice_utils import (flatten_nested_dict, make_grid,
                                           setup_training, update_config_dict)
...
def main(config):
    ...
    with wandb.init(**wandb_kwargs) as run:
        ...
        params = flow.init(
            key,
            jnp.zeros((config.batch_size, config.im_size**2)),
            jnp.array(config.batch_size),
        )
        ...
        if config.wandb.log_artifact:
            artifact_name = (
                f"{config.alpha}_{config.n_bits}_{config.im_size}_{config.dataset}"
            )
